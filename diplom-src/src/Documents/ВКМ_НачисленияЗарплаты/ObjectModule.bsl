//НачалоДоработки. Барышникова Ю.А 11.11.2024 
//Начисление з/п
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	...
	Процедура СформироватьДвиженияОснНачисления()  
		...
		Для Каждого Строка Из ВКМ_Начисления  Цикл  
			...
			Если Строка.ВКМ_ВидРасчета = ПланыВидовРасчета.ВКМ_ОсновныеНачисления.ВКМ_Оклад Тогда     
				...
				Движение.ВКМ_Сотрудник = Строка.ВКМ_Сотрудник;
				Движение.ВКМ_ОтпускФакт = (Строка.ВКМ_ДатаОкончания-Строка.ВКМ_ДатаНачала+86400)/86400;
	...
	Процедура СформироватьДвиженияУдержания()  
		...
		Для Каждого Строка Из ВКМ_Начисления Цикл 
			...
			ПроверкаСотрудника = СотрудникЕсть.Найти(Строка.ВКМ_Сотрудник); 
			...
			Если ПроверкаСотрудника = Неопределено Тогда 
				...
				СотрудникЕсть.Добавить(Строка.ВКМ_Сотрудник);  
	...
	Процедура РассчитатьОкладИВыполненныеРаботы()    
		...
		|	ЕСТЬNULL(ВКМ_ВыполненныеСотрудникомРаботыОбороты.ВКМ_СуммаКОплатеОборот, 0) КАК СуммаКОплатеЗаРаботы
		|ИЗ
		...
		|		ПО ВКМ_ОсновныеНачисленияДанныеГрафика.ВКМ_Сотрудник = ВКМ_ВыполненныеСотрудникомРаботыОбороты.ВКМ_Сотрудник";
		...
		Пока Выборка.Следующий() Цикл
			...
			Движение.ВКМ_Оклад = Выборка.Оклад;

			Если Выборка.План = 0 Тогда
				Движение.ВКМ_Начислено = 0;

			Иначе
				Движение.ВКМ_Начислено = Выборка.Оклад * Выборка.Факт / Выборка.План + Выборка.СуммаКОплатеЗаРаботы;  
			КонецЕсли;

			Движение.ВКМ_КоличествоРабочихДней = Выборка.Факт; 
			...
			ДвижениеВз.ВКМ_Сумма = Движение.ВКМ_Начислено;
	...
	Процедура РассчитатьОтпуск()  
		...
		|	ЕСТЬNULL(ВКМ_ОсновныеНачисленияБазаВКМ_ОсновныеНачисления.ВКМ_НачисленоБаза, 0) КАК БазаОсн,
		|	ЕСТЬNULL(ВКМ_ОсновныеНачисленияБазаВКМ_ДополнительныеНачисления.ВКМ_НачисленоБаза, 0) КАК БазаДоп,
		|	ВКМ_ОсновныеНачисленияБазаВКМ_ОсновныеНачисления.ВКМ_КоличествоРабочихДней КАК КоличествоРабочихДнейБаза,
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.КоличествоРабочихДнейБаза = NULL Тогда        
				...
				Продолжить;
			Иначе
				Движение.ВКМ_Начислено = (Выборка.БазаОсн + Выборка.БазаДоп) / Выборка.КоличествоРабочихДнейБаза * Выборка.Факт; 
			КонецЕсли;
	...
	Процедура РассчитатьНДФЛ() 
		...
		|	ЕСТЬNULL(ВКМ_УдержанияБазаВКМ_ОсновныеНачисления.ВКМ_НачисленоБаза, 0) КАК НачисленоБаза,
		...
		Запрос.УстановитьПараметр("Ссылка", Ссылка);   
		Запрос.УстановитьПараметр("Удержание",  ПланыВидовРасчета.ВКМ_Удержания.ВКМ_НДФЛ);  
		...
		Пока Выборка.Следующий() Цикл
			...
			ДвижениеУдержание.ВКМ_НДФЛ = Выборка.НачисленоБаза/100*13; 
			...
			ДвижениеВз.ВКМ_Сумма = ДвижениеУдержание.ВКМ_НДФЛ;   
