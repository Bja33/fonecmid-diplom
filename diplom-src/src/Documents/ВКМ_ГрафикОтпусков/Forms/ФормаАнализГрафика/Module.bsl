...
&НаСервере
Процедура ПрочитатьДанныеИзВременногоХранилищаНаСервере(АдресВХ)
	
	ТаблицаЗначенийВКМ_ОтпускаСотрудников  = ПолучитьИзВременногоХранилища(АдресВХ);
	
	Для каждого Стр Из ТаблицаЗначенийВКМ_ОтпускаСотрудников  Цикл  
